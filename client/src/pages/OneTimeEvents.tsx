...
export default function OneTimeEvents() {
  ...
  // Sort events by age for display
    ...
    <div className="space-y-6">
      ...
      {events.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4">
            <p className="text-[10px] text-emerald-600 uppercase tracking-wide">Total Inflows</p>
            <p className="text-2xl font-bold text-emerald-700 mt-0.5 tabular-nums">
              +{formatCurrency(totalInflows, true)}
            </p>
          </div>
          <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4">
            <p className="text-[10px] text-red-500 uppercase tracking-wide">Total Outflows</p>
            <p className="text-2xl font-bold text-red-600 mt-0.5 tabular-nums">
        ...
        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Events are applied to the specified account at the start of that age's projection year.
            </p>
          </div>
          <button
            onClick={() => addEvent()}
            className="flex items-center gap-2 px-3 py-2 bg-[#1B4332] text-white text-xs font-semibold rounded-lg hover:bg-[#2D6A4F] transition-colors"
          >
        ...
        {events.length === 0 ? (
          ...
          <>
            ...
            <div className="grid items-center px-6 py-2 bg-slate-50 text-[10px] font-semibold text-slate-400 uppercase tracking-wide"
              style={{ gridTemplateColumns: "60px 1fr 130px 110px 32px" }}>
              <span>Age</span>
            ...
            {sortedEvents.map((event) => {
              ...
              return (
                <div
                  key={event.id}
                  className="grid items-center px-6 py-2.5 border-t border-slate-50 hover:bg-slate-50/50 transition-colors"
                  style={{ gridTemplateColumns: "60px 1fr 130px 110px 32px" }}
                >
                  {/* Age */}
                  <EditableNumber
                    value={event.age}
                    onChange={(v) => updateEvent(event.id, "age", Math.max(inputs.currentAge, v))}
                    integer
                  ...
                  <select
                    value={event.account}
                    onChange={(e) => updateEvent(event.id, "account", e.target.value as OneTimeEvent["account"])}
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
          ...
          <div className="space-y-1">
            {projection
              ...
              .map((r) => (
                <div key={r.age} className="flex items-center gap-3 py-1.5 border-b border-slate-50 last:border-0">
                  <span className="text-xs font-bold text-slate-500 w-16 flex-shrink-0">Age {r.age}</span>
                  <span className="text-xs text-slate-500 w-16 flex-shrink-0">{r.year}</span>
                  <span className={cn(
